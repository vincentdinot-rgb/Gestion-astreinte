...
med_absent = st.sidebar.selectbox("Médecin", MEDECINS, key="abs_med")
...
if st.sidebar.button("Valider le congé", type="secondary"):
    for i in range((date_fin - date_deb).days + 1):
        st.session_state['absences'].append({"medecin": med_absent, "date": date_deb + datetime.timedelta(days=i)})
    st.sidebar.success("Absence enregistrée")

st.sidebar.markdown("---")
st.sidebar.subheader("2. Jours préférentiels (Semaine)")
med_pref = st.sidebar.selectbox("Médecin concerné", MEDECINS, key="pref_med")
...
with col1:
    ...
    with col_a:
        ...
        mois_cible = st.number_input("Mois", min_value=1, max_value=12, value=datetime.date.today().month)
    with col_b:
    ...
    if st.button("🚀 Générer le planning", type="primary", use_container_width=True):
        with st.spinner("Calcul des équilibres en cours..."):
            df_sec, df_comp = generer_planning(
                ...
                historique_dict,
                st.session_state['feries'] # Nouvel argument passé au moteur
            )
...
with col2:
    ...
    if not st.session_state['df_secteurs'].empty:
        ...
        st.download_button("📥 Télécharger (Excel/CSV)", data=csv, file_name="Planning_OHS.csv", mime="text/csv")
